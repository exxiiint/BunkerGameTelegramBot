...
class Lobby(Base):
    ...
    owner_tg_user_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("users.tg_user_id"), index=True
    )
    status: Mapped[LobbyStatus] = mapped_column(
        Enum(LobbyStatus), default=LobbyStatus.OPEN, index=True
    )
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    owner: Mapped[User] = relationship()
    members: Mapped[list["LobbyMember"]] = relationship(
        back_populates="lobby", cascade="all, delete-orphan"
    )
...
class LobbyMember(Base):
    ...
    lobby_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("lobbies.id", ondelete="CASCADE"), index=True
    )
...
class Game(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    lobby_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("lobbies.id", ondelete="SET NULL"), nullable=True, index=True
    )

    status: Mapped[GameStatus] = mapped_column(
        Enum(GameStatus), default=GameStatus.ACTIVE, index=True
    )
    phase: Mapped[GamePhase] = mapped_column(
        Enum(GamePhase), default=GamePhase.BUNKER_CHOICE, index=True
    )
    ...
    turn_seat: Mapped[int] = mapped_column(
        Integer, default=1
    )  # whose turn right now in reveal circle

    # Voting state within a round
    vote_no: Mapped[int] = mapped_column(Integer, default=0)  # 0=not voting, 1 or 2
    vote_attempt: Mapped[int] = mapped_column(
        Integer, default=0
    )  # 0=not started, 1=first, 2=revote
    ...
    bunker_cards: Mapped[list["BunkerCard"]] = relationship(
        back_populates="game", cascade="all, delete-orphan"
    )
...
class GamePlayer(Base):
    ...
    __table_args__ = (
        UniqueConstraint("game_id", "seat_no", name="uq_game_seat"),
        UniqueConstraint("game_id", "tg_user_id", name="uq_game_user"),
    )
    ...
    status: Mapped[PlayerStatus] = mapped_column(
        Enum(PlayerStatus), default=PlayerStatus.ALIVE, index=True
    )
...
class Vote(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "game_id", "round_no", "vote_no", "attempt", "voter_seat", name="uq_vote_once"
        ),
    )
